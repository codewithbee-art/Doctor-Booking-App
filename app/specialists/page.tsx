  ...
  return (
    <div className={`rounded-xl border bg-white p-5 shadow-sm ${isPast ? "border-border" : "border-primary/20 hover:border-primary/40"} transition-colors`}>
      <div className="flex items-start justify-between gap-3 mb-3">
        <div>
          <h3 className="font-heading text-base font-bold text-text-primary">{s.specialist_name}</h3>
          <p className="font-body text-sm text-primary font-semibold">{s.specialization}</p>
        </div>
        {!isPast && (
          <span className="flex-shrink-0 rounded-full bg-accent/10 px-2.5 py-0.5 font-body text-xs font-semibold text-accent">
            Upcoming
          </span>
        )}
      </div>
      ...
      <div className="space-y-1.5 font-body text-sm">
        <div className="flex items-center gap-2">
          <svg className="h-4 w-4 text-text-secondary flex-shrink-0" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
