...
export default function ProductDetailPage() {
  ...
  const canAdd = product ? product.stock_status !== "out_of_stock" && product.stock_status !== "hidden" : false;
  ...
  const handleAddToCart = useCallback(() => {
    ...
    addItem({
      ...
      requires_consultation: product.requires_consultation,
      allow_delivery: product.allow_delivery,
      allow_pickup: product.allow_pickup,
    }, qty);
    setAdded(true);
    setTimeout(() => setAdded(false), 2000);
  }, [product, canAdd, addItem, qty]);
                ...
                {canAdd ? (
                  <>
                    <div className="flex items-center gap-3 mb-3">
                      <label htmlFor="qty-select" className="font-body text-sm font-medium text-text-primary">Quantity</label>
                      <div className="flex items-center rounded-lg border border-border">
                        <button
                          type="button"
                          onClick={() => setQty((q) => Math.max(1, q - 1))}
                          className="px-3 py-2 font-body text-lg text-text-primary hover:bg-bg-light rounded-l-lg transition-colors"
                        ...
                        <input
                          ...
                          className="w-14 border-x border-border bg-white py-2 text-center font-body text-sm text-text-primary focus:outline-none"
                          aria-label="Quantity"
                        />
                        <button
                          type="button"
                          onClick={() => setQty((q) => Math.min(99, q + 1))}
                          className="px-3 py-2 font-body text-lg text-text-primary hover:bg-bg-light rounded-r-lg transition-colors"
                    ...
                    <button
                      ...
                      className={`inline-flex items-center gap-2 rounded-xl px-7 py-3.5 font-body text-base font-semibold text-white shadow-sm transition-colors ${
                        added ? "bg-green-600" : "bg-accent hover:bg-accent-hover"
                      }`}
                    ...
                    >
                      ...
                      {added ? "Added to Cart!" : inCart > 0 ? `Add More (${inCart} in cart)` : "Add to Cart"}
                    </button>
                    {inCart > 0 && (
                      <Link href="/cart" className="ml-3 font-body text-sm font-semibold text-primary underline underline-offset-2 hover:text-secondary transition-colors">
                        View Cart
                      </Link>
                    )}
                  </>
                ) : (
                  <p className="font-body text-base font-semibold text-red-600">Currently Out of Stock</p>
                )}
                {product?.requires_consultation && canAdd && (
                  <p className="mt-2 font-body text-xs text-purple-700">This product requires a doctor&apos;s consultation. Your order will be reviewed by our team before confirmation.</p>
                )}
              </div>
            </div>
          </div>
